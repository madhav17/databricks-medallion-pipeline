...
def main() -> int:
    ...
    result = ingest_customers(spark, config, run_id)
    write_ingestion_metadata(spark, config, [result])

    if result.status != "SUCCESS":
        raise BronzeIngestionError(result.error_message or "Customer ingestion failed")

    print(
        f"customers ingestion succeeded: "
